...
(define (domain Ejercicio7)
  ...
  (:functions
    ...
    (total-player-score ?x - Dealer); puntuación total del jugador 'x'
    ...
    (:action turn-right; Girar a la derecha
     ...
     (:action pick-up; Coger un item
       :parameters (?p - Picker ?o - item ?z - zone)
     ...
     (:action give-NPC; Entregar un item a un personaje
       :parameters (?p - Dealer ?o - item ?z - zone ?n - NPC)
       :precondition (and (at ?z ?p) (at ?z ?n) (has-object ?p ?o) (not(is-bikini ?o)) (not(is-sneaker ?o)) (> (pocket-space ?n) 0))
       :effect(and (not(has-object ?p ?o)) (hands-empty ?p) (increase (total-player-score ?p) (score ?o ?n)) (increase (total-score) (score ?o ?n)) (decrease (pocket-space ?n) 1))
     )

     (:action give-dealer; Entregar un item a un dealer
       :parameters (?p - Picker ?o - item ?z - zone ?n - Dealer)
       :precondition (and (at ?z ?p) (at ?z ?n) (has-object ?p ?o) (hands-empty ?n))
       :effect(and (not(has-object ?p ?o)) (hands-empty ?p) (has-object ?n ?o) (not(hands-empty ?n)) (WHEN(is-sneaker ?o) (and(sneakers ?n) (not(sneakers ?p)))) (WHEN(is-bikini ?o) (and(bikini ?n) (not(bikini ?p)))))
     )
    )
